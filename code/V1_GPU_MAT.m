function [CMQmat,CMQ]=V1_GPU_MAT(bcell,smcell,interval,cellnum,cellnumjy,smcellnum,widght,tp1,tp2,t,h,OSS)
% gttao=[1,100,200,501,555,1000]';
io=size(bcell,2);
for i=1:tp2-1
    ...
    [gpu_image_t1,gpu_image_t2]=arrayfun(@V1_GPU_mapping,gpu_cdnt1,gpu_cdnt2,t,h,OSS);
    image=zeros(size(cdnt));
    for j=1:io
        eval(['image_t(:,j)=gather(gpu_image_t',num2str(j),');']);
    end
    fttao=F(image_t,bcell,interval,widght,cellnumjy);
    temp1=tp1*smcellnum;
    mid1=repelem(gttao,smcellnum);
    unq=zeros(temp1,2);
    unq(:,1)=mid1;
    unq(:,2)=fttao;
...
fttao=F(image_t,bcell,interval,widght,cellnumjy);
mid1=repelem(gttao,smcellnum);
temp1=length(gttao)*smcellnum;
unq=zeros(temp1,2);
unq(:,1)=mid1;
unq(:,2)=fttao;
eval(['[xishu_',num2str(tp2),',~,midt_',num2str(tp2),']=unique(unq,''rows'');']);
eval(['[lg(tp2),~]=size(xishu_',num2str(tp2),');']);
eval(['midt',num2str(tp2),'=histc(midt_',num2str(tp2),',[1:lg(',num2str(tp2),')]'')/smcellnum;']);
